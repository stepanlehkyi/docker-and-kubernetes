# Запуск з Docker використовуючи Windows PowerShell:

1. Створіть порожню папку "redis-storage";
2. Використовуючи Windows PowerShell, напишіть команду:
   - docker compose up -d
  Це створить python web app на порті 8080.
  Є опція змінити цей порт (Windows Power Shell):
   - $env:PYTHON_WEB_APP_PORT="<Ваш порт>"; docker compose up -d
   або якщо Ви користуєтеся Windows CMD, то:
   - set PYTHON_WEB_APP_PORT=<Ваш порт>&& docker compose up -d
3. Зайдіть на localhost:<Ваш порт> і переконайтеся, що все працює правильно.
...
Compose файлу немає, його потрібно написати самостійно
